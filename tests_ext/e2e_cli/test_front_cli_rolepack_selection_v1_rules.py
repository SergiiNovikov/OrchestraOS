...
def test_env_selects_v1_rules_and_produces_final_response() -> None:
    summary, output, runtime_error, manifest = _run_cli(
        "hello",
        env_overrides={"ORCHESTRA_ROLEPACK": "v1_rules"},
    )
    ...
    assert isinstance(output.get("results"), list) and len(output["results"]) > 0


def test_v1_rules_determinism_n3() -> None:
    s1, o1, e1, m1 = _run_cli("hello", env_overrides={"ORCHESTRA_ROLEPACK": "v1_rules"})
    s2, o2, e2, m2 = _run_cli("hello", env_overrides={"ORCHESTRA_ROLEPACK": "v1_rules"})
    s3, o3, e3, m3 = _run_cli("hello", env_overrides={"ORCHESTRA_ROLEPACK": "v1_rules"})
